...
全局安装将安装到 nodejs 的安装目录。
可以直接在命令行里使用。

npm install express      # 本地安装
本地安装将安装包放在 `./node_modules` 下（运行npm命令时所在的目录）

npm package.json 中 两个 dependencies 和 devDependencies 的区别
...
在使用 CORS 跨域的方法中，如果程序员偷懒将Access-Control-Allow-Origin设置为：Access-Control-Allow-Origin:*允许任何来自任意域的跨域请求，那么就存在被DDos攻击的可能。
