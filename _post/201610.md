...
-----
在使用
<input class="rem-check" id="remember_me" type="checkbox">
<label class="rem-label" for="remember_me"></label>
的时候，关于for与id的关联
关联后的css样式中，就可以进行样式的联通

.login-panel li input.rem-check + label.rem-label:hover {
    background: url(../img/minimal.png) -20px 0 no-repeat;  
}
.login-panel li input.rem-check:hover + label.rem-label:after{
    color: #000;
}
.login-panel li input.rem-check:checked + label.rem-label {
    background: url(../img/minimal.png) -40px 0 no-repeat;
}
.login-panel li input.rem-check:checked + label.rem-label:after{
    color: #000;
}
请仔细分析这段代码在hover，checked的时候，label与input的关系



映客的产品观就是“简单极致”，没做太多复杂功能。这点和微信有着异曲同工之处“极简方能不被超越”。

                                                                             


*html{_background-image:url(about:blank);_background-attachment:fixed;}，什么意思？
...
关于页面异步加载数据显示loading透明小弹窗
loadList : function(page_index, page_size, request) {
        var that = this;
        dom = that.getDom();
        dom.loading.show();
        if (request === undefined) {
            request = that.getRequest(dom);
        }
        $ajax({
            url : SITEURL + 'recharge_palmid/getlist?dtype=json' + request + '&page_index=' + page_index + '&page_size=' + page_size,
            type : "GET",
            dataType : 'json',
            success : function(rdata) {
                if(rdata.code == '-1'){
                    bootbox.alert(rdata.msg,function(){
...
包括 200 个来自 Glyphicon Halflings 的字体图标。Glyphicons Halflings 一般不允许免费使用，但是他们的作者允许 Bootstrap 免费使用。为了表示感谢，希望你在使用时加上 Glyphicons 的链接。
...
YUI,Yahoo UI,Yahoo!UI Library,是一个开放源代码的JavaScript函数库，为了能建立一个高互动的网页，它采用了AJAX，DHTML和DOM等程式码技术。它包含了许多CSS资源。使用授权为BSD许可证。与2014-08-29，由于缺乏积极维护，过分庞大的library不合时代...等原因停止维护。
